...
on sauvegarde les valeurs de $\pi$ trouvées dans un tableau. Ensuite on utilise
la moyennes et le tableaux de valeurs de $\pi$ pour calculer $S^{2}(n) =
\frac{\sum_{i = 1}^{n}{X_{i} - \overline{X}(n)}}{n - 1}$, qui est un estimateur
sans biais de la variance. Enfin on utilise une loi de Student pour calculer le
taux d'erreur.
