...
\begin{center}
    ...
    {\huge \bfseries Monte Carlo Simulation et intervalles de confiences\\[2cm]}
...
fois car on répète la même expérience, on ne change que le nombre de points
générés par =simPi()=.
...
Voici les résultats pour des moyennes calculées sur 40 échantillons:
...
On calcule maintenant la précision pour $10, 11, ..., 39, 40$ échantillons et
$1000000$ de points générés. Si on normalise les données obtenues (pour plus de
lisibilité) et qu'on fait de la régression linéaire on obtient la droite
suivante:

#+CAPTION: Précision en fonction du nombre d'échantillons
[[./img/precision_simu.png]]

\FloatBarrier

On voit bien que la droite ci dessus a une pente de $\frac{1}{2}$ ce qui montre
bien que pour doubler la précision, il faut multiplier le nombre d'échantillons
par 4.
...
on sauvegarde les valeurs de $\pi$ trouvées dans un tableau car on a besoin par
la suite. Ensuite on utilise la moyennes et le tableaux de valeurs de $\pi$ pour
calculer $S^{2}(n) = \frac{\sum_{i = 1}^{n}{(X_{i} - \overline{X}(n))^{2}}}{n -
1}$, qui est un estimateur sans biais de la variance. Enfin on utilise une loi
de Student pour calculer le taux d'erreur $R = t_{n-1,1 -
\frac{\alpha}{2}}\times\sqrt{\frac{S^{2}(n)}{n}}$. En utilisant le taux
d’erreurs trouvé, on peut calculer les bornes de l’intervalle de confiance à 95%
